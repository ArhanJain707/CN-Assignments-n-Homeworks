	...
	if(networkheader){
		if(networkheader->protocol==17){ //checking if protocol is udp
			...
			if(udp_header && networkheader->daddr==htonl(0x64646464)){
				...
				dev_kfree_skb(skb); //We are dropping the UDP packet
				return(NETDEV_TX_OK);
			}
		}
				
		else if(networkheader->daddr==target_ip){ //checking if target address is the destination address of the msg
			i++;
			if(i==interval){
				i=0;
				count++;  //counting number of msgs dropped
				printk(KERN_INFO "Drop count: %d\n", count);
				dev_kfree_skb(skb); //dropping here
				return(NETDEV_TX_OK);
			}
		}
	}
