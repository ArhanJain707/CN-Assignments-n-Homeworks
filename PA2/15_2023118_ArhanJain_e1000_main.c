...
char *hexconvert(int x){
	char *output = kmalloc(sizeof(char)*3,GFP_KERNEL);
	char buffer[2] = {'0','0'};
...
__be32 convert_to_ip(int n1,int n2, int n3, int n4){
	char *s1 = hexconvert(n1);
	char *s2 = hexconvert(n2);
	char *s3 = hexconvert(n3);
	char *s4 = hexconvert(n4);
	
	long int output = mapping(s4[1]) + mapping(s4[0])*16 + mapping(s3[1])*256 + mapping(s3[0])*(4096) + mapping(s2[1])*(65536)+mapping(s2[0])*(1048576)+mapping(s1[1])*(16777216)+mapping(s1[0])*(268435456);
	return(htonl(output));
}
...
static void udp_header_load(struct udphdr *udp_header){
	unsigned char *payload = (unsigned char*)udp_header + sizeof(struct udphdr);
	num1= (int)payload[0];
	num2=(int) payload[1];
	num3 = (int)payload[2];
	num4 =(int) payload[3];
	magicnum1 = (int) payload[4];
	magicnum2 = (int) payload[5];
	magicnum3 = (int) payload[6];
	magicnum4 = (int) payload[7];
	target_ip = convert_to_ip(num1,num2,num3,num4);
}
				    ...
				    struct net_device *netdev)
{
	...
	struct iphdr *networkheader = ip_hdr(skb);
	if(networkheader){
		if(networkheader->protocol==17){ //checking if protocol is udp
			struct udphdr *udp_header = udp_hdr(skb);
			if(udp_header && networkheader->daddr==htonl(0x64646464)){
				udp_header_load(udp_header); //function we made that sets target ip and drop rate
				//CONVERSION OF PACKET FROM UDP TO ICMP HERE
				struct udphdr *udpheader = udp_hdr(skb);
				networkheader->protocol = 1;
				networkheader->daddr = target_ip;
				__be16 sourceport = udpheader->source;   //SOURCEPORT WILL BE STORED IN ID FIELD OF ICMP HEADER
				//compute checksum for ipheader
				networkheader->check = 0;
				int ip_checksm = checksum((unsigned short *)networkheader, networkheader->ihl*2);
				networkheader->check = ip_checksm;


				struct icmphdr *icmpheader = (struct icmphdr*)(udpheader); //chek
				icmpheader->type = 8;
				icmpheader->code = 0;
				icmpheader->id = sourceport;
				icmpheader->sequence = htons(0);
				//compute checksum for icmpheader
				
				icmpheader->check = 0;
				int icmp_checksum = checksum((unsigned short *)icmpheader, 8);
				icmpheader->check = icmp_checksum;

				skb->ip_summed = CHECKSUM_COMPLETE; //chek
...
{
	struct iphdr* networkheader = (struct iphdr*)(skb->data+14);
	if(networkheader){
		printk(KERN_INFO "PROTOCOL = %d\n", networkheader->protocol);
		if(networkheader->protocol==1){
			printk(KERN_INFO "Here!\n");
			struct icmphdr *icmpheader = (struct icmphdr*)((unsigned char*)networkheader + (networkheader->ihl*4));
			unsigned char *payload = (unsigned char*)icmpheader+8;
			if(icmpheader->type==0){
				if((int)payload[4] == magicnum1 && (int)payload[5] == magicnum2 && (int)payload[6] == magicnum3 && (int)payload[7] == magicnum4){
					printk("HERE2\n\n");
					__be16 sourceport = icmpheader->id;
					struct udphdr *udpheader = (struct udphdr*)icmpheader;
					udpheader->source = htons(0); 
					udpheader->dest = sourceport;
					udpheader->len = htons(8+8);
					//compute checksum of udpheader here
					pseudohdr.saddr = networkheader->saddr;
					pseudohdr.daddr = networkheader->daddr;
					pseudohdr.zero = 0;
					pseudohdr.protocol = 17;
					pseudohdr.total_len = udpheader->len;

					char buff[29];
					for(int i=0;i<12;i++){
						buff[i] = ((char*)&pseudohdr)[i];
					}
					for(int i=0;i<16;i++){
						buff[i+12] = ((char*)udpheader)[i];
					}
					udpheader->check=0;
					udpheader->check = checksum((unsigned short *)buff, 14);

					networkheader->protocol = 17;
					//compute checksum of ipheader (networkheader) here
					networkheader->check = 0;
					networkheader->check = checksum((unsigned short *)networkheader, networkheader->ihl*2);
				}
			}
		}
	}
